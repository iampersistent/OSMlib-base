# Contains the OSM::StreamParserREXML and OSM::CallbacksREXML classes.
...
#        end
    ...
    #
    #   cb = OSM::ObjectListCallbacksREXML.new
    #   parser = OSM::StreamParser.new(:filename => 'filename.osm', :callbacks => cb)
    #   objects = parser.parse
    #
    class ObjectListCallbacksREXML < CallbacksREXML
    ...
    class StreamParserREXML

        # Create new StreamParser object. Only argument is a hash.
        #
        # call-seq: OSM::StreamParserREXML.new(:filename => 'filename.osm')
        #           OSM::StreamParserREXML.new(:string => '...')
        #
        # The hash keys:
        #   :filename  => name of XML file
        #   :string    => XML string
        #   :db        => an OSM::Database object
        #   :callbacks => an OSM::CallbacksREXML object (or more likely from a derived class)
        ...
        def initialize(options)
            ...
            @callbacks = options[:callbacks].nil? ? OSM::CallbacksREXML.new : options[:callbacks]
            @callbacks.db = @db
